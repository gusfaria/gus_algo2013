...
void VectorField::setPerlin( float scale) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ...
            flowList[y][x].set( ofVec2f( cos(noise)*500, sin(noise)*500 ) );
        }
    }
}
...
// Jennifer Presto:
//https://github.com/jenniferpresto/presto_algo2013/tree/master/week07_02_vectorField

void VectorField::useImage(ofImage _img){
    img = _img;
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            
            ofColor tmpColor = img.getColor(x * resolution + (resolution * 0.5), y * resolution + (resolution * 0.5));
            ...
            if (brightness <= 4) {
                newDir.x = cos(angle) * -10;
                newDir.y = sin(angle) * -10;
            } else {
                newDir.x = cos(angle) * 10;
                newDir.y = sin(angle) * 10;
            }

            flowList[y][x].set(newDir);
